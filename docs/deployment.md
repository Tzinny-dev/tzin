...
- [x] **Release automation (assisted)** — `scripts/release.mjs <patch|minor|major>
  ...
  prepends a CHANGELOG skeleton, commits + tags. Publish itself is MANUAL
  (see next item) — the script only prints the commands.
- [x] **Tag-triggered CI** — `.github/workflows/release.yml` runs `npm ci` +
  `npm run build` + `npm test` on every `v*` tag. Publishing is NOT in CI:
  it happens locally with the npm token already in `~/.npmrc` (no secret in
  the repo, no OTP). If you want CI to publish, uncomment the `publish`
  job in the workflow and register a Trusted Publisher (OIDC) for both
  packages at npmjs.com.
